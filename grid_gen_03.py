...
# ncoll=g_janet.plot_nodes(values=g_janet.nodes['fixed'],cmap='winter')
#mask=(g_janet.edges['cells'].min(axis=1)<0)
# g_janet.plot_halfedges(labeler=lambda j,side: str(g_janet.edges['cells'][j,side]),mask=mask)
plt.axis('equal')
...
af.grid.write_ugrid('janet/janet-out-stompy17.nc',overwrite=True)
...
g=unstructured_grid.UnstructuredGrid.from_ugrid('janet/janet-out-stompy17.nc')
